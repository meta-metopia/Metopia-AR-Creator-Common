...
public class SupabaseStorageClient: NetworkRequestStorageProtocol {
  ...
  public func upsertObject(at path: String, file: File, isUpdate: Bool) async throws {
    if !isUpdate {
      _ = try await client.storage.from(id: "metopia").upload(
        path: path,
        file: .init(
          name: file.name, data: file.data, fileName: file.fileName, contentType: file.contentType),
        fileOptions: nil)
    } else {
      _ = try await client.storage.from(id: "metopia").update(
        path: path,
        file: .init(
          name: file.name, data: file.data, fileName: file.fileName, contentType: file.contentType),
        fileOptions: nil)
