...
public class SupabaseDownloadUploader: DownloadUploaderProtocol {
  ...
  public func download(file: DownloadableProtocol, type: DownloadTypeProtocol) async throws -> Data?
  {
    ...
    if !FileManager.default.fileExists(atPath: downloadFolder.path) {
      try! FileManager.default.createDirectory(
        atPath: downloadFolder.path, withIntermediateDirectories: true)
    }
    ...
    if !FileManager.default.fileExists(atPath: downloadDestination.path) {
      ...
      let task = AF.download(downloadSource, method: .get, to: destination)
  ...
  public func upload(
    file: UploadableProtocol, type: UploadTypeProtocol, data: Data, isUpdate: Bool = false
  ) async throws -> URL {
    let storageClient = SupabaseStorageClient(url: endpoint, key: key)

    try await storageClient.upsertObject(
      at: file.uploadDestination(type: type)!.absoluteString,
      file: file.uploadFile(type: type, data: data)!, isUpdate: isUpdate)
