...
class MockSupabaseAuthenticationClient: NetworkRequestAuthProtocol {
  ...
  func signIn(email: String, password: String) async throws -> MetopiaARCreatorCommon.Session {
    return MetopiaARCreatorCommon.Session(
      accessToken: "a", tokenType: "b", expiresIn: 1, refreshToken: "c", user: User(id: UUID()))
  }
...
final class SupabaseClientTests: XCTestCase {
  ...
  func testFunctionURL() throws {
    let client = SupabaseServiceClient(url: URL(string: "https://abc.supabase.co")!, key: "abcde")
    XCTAssertEqual(client.functionUrl.absoluteString, "https://abc.functions.supabase.co")
  }
